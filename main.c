...
#define RTC_ON_TICKS (10 * 8)     // 120 ticks
#define RTC_SLEEP_TICKS (10 * 8)  // 160 ticks
#define RTC_PRESCALER 4095        // Prescaler for 1 second ticks

NRF_BLE_GATT_DEF(m_gatt); /**< GATT module instance. */
static uint16_t m_conn_handle = BLE_CONN_HANDLE_INVALID;
static uint16_t m_ble_nus_max_data_len = BLE_GATT_ATT_MTU_DEFAULT - OPCODE_LENGTH -
 ...
 *
 * @details This function receives a single character from the app_uart module
 * and appends it to a string. The string is sent over BLE when the last
 * character received is a 'new line' '\n' (hex 0x0A) or if the string reaches
...
static void uart_init(void)
{
	...
	APP_UART_FIFO_INIT(&comm_params, UART_RX_BUF_SIZE, UART_TX_BUF_SIZE, uart_event_handler, APP_IRQ_PRIORITY_LOWEST,
	                   err_code);
...
void handle_rtc_events(void)
{
	...
	if (m_rtc_on_flag)
	{
		...
		if (m_device_active)
		{
			...
			NRF_LOG_INFO("Fecha: %04u-%02u-%02u Hora: %02u:%02u:%02u", now.year, now.month, now.day, now.hour,
...
void rtc_init(void)
{
	...
	nrfx_rtc_int_enable(&m_rtc, NRF_RTC_INT_COMPARE0_MASK | NRF_RTC_INT_COMPARE1_MASK | NRF_RTC_INT_COMPARE2_MASK);
 ...
 * prints the characters out on UART. If @ref ECHOBACK_BLE_UART_DATA is set, the
 * data is sent back to sender.
 */
...
static void idle_state_handle(void)
{
	if (NRF_LOG_PROCESS() == false)
	{
		if (!m_device_active)
		{
			// Entrar en bajo consumo solo cuando está inactivo
			nrf_pwr_mgmt_run();
		}
		else
		{
			// Pequeña espera activa cuando está despierto
			//__WFE();
			//__SEV();
			//__WFE();
			sd_app_evt_wait();
