...
{
	uint16_t record_key =
		(valor_type == TIEMPO_ENCENDIDO) ? TIME_ON_RECORD_KEY : TIME_SLEEP_RECORD_KEY;
	fds_record_t record = {
		.file_id = TIME_FILE_ID, .key = record_key, .data.p_data = &valor, .data.length_words = 1};
...
uint32_t read_time_from_flash(valor_type_t valor_type, uint32_t default_valor)
{
	...
	record_key = (valor_type == TIEMPO_ENCENDIDO) ? TIME_ON_RECORD_KEY : TIME_SLEEP_RECORD_KEY;
...
#define FIRST_CONN_PARAMS_UPDATE_DELAY                                                             \
	APP_TIMER_TICKS(5000) /**< Time from initiating event (connect or start of             \
                 notification) to first time sd_ble_gap_conn_param_update is   \
                 called (5 seconds). */
#define NEXT_CONN_PARAMS_UPDATE_DELAY                                                              \
	APP_TIMER_TICKS(30000) /**< Time between each call to sd_ble_gap_conn_param_update     \
...
static ble_gap_addr_t m_target_periph_addr; // No const, se inicializa en tiempo de ejecución
static uint8_t mac_address_from_flash[6] = {0}; // Arreglo de 6 bytes para la MAC
...
static void load_mac_from_flash(void)
{
	...
	// Busca el registro en la memoria flash
	ret_code_t err_code = fds_record_find(MAC_FILE_ID, MAC_RECORD_KEY, &record_desc, &ftok);
	if(err_code == NRF_SUCCESS)
	{
		err_code = fds_record_open(&record_desc, &flash_record);
		if(err_code == NRF_SUCCESS)
	...
	{
		NRF_LOG_WARNING("No se encontro una MAC en la memoria flash. Usando valor "
						"predeterminado.");
...
static void delete_old_records(void)
{
	...
	while(fds_record_find(MAC_FILE_ID, MAC_RECORD_KEY, &record_desc, &ftok) == NRF_SUCCESS)
	{
	...
	if(p_evt->type == BLE_NUS_EVT_RX_DATA)
	{
		...
		if(p_evt->params.rx_data.length < sizeof(message))
		{
			   ...
			   message[2] == '1')
			{
				...
				case 8: // Comando 08: Escribe en la memoria flash la fecha y hora, formato YYYYMMDDHHMMSS
				{
					NRF_LOG_RAW_INFO("\n\n\x1b[1;36m--- Comando 08 recibido: Guardar fecha y hora\x1b[0m");

					// Espera recibir: 11108YYYYMMDDHHMMSS (total 5+14=19 bytes)
					if (p_evt->params.rx_data.length >= 19) // 5 de comando + 14 de fecha/hora
					{
						const char *fecha_iso = &message[5]; // Apunta al inicio de la fecha/hora

						datetime_t dt;
						// Parseo seguro de la cadena: YYYYMMDDHHMMSS
						if (sscanf(fecha_iso, "%4hu%2hhu%2hhu%2hhu%2hhu%2hhu",
								   &dt.year, &dt.month, &dt.day, &dt.hour, &dt.minute, &dt.second) == 6)
						{
							NRF_LOG_RAW_INFO("\nFecha y hora recibida: %04u-%02u-%02u %02u:%02u:%02u",
											 dt.year, dt.month, dt.day, dt.hour, dt.minute, dt.second);

							// Guarda la estructura en flash (ajusta según tu función)
							write_date_to_flash(&dt);
						}
						else
						{
							NRF_LOG_WARNING("Formato inválido. Se esperaba YYYYMMDDHHMMSS.");
						}
					}
					else
					{
						NRF_LOG_WARNING("Comando 08 recibido con datos insuficientes. Se esperaban 14 dígitos para fecha y hora en formato YYYYMMDDHHMMSS.");
					}
					break;
				}
