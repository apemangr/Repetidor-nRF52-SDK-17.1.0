...
static void fds_evt_handler(fds_evt_t const *p_evt) {
  if (p_evt->id == FDS_EVT_INIT) {
    ...
    if (p_evt->result == NRF_SUCCESS) {
      NRF_LOG_RAW_INFO(
          "\n\x1b[1;32m>> Registro actualizado correctamente!\x1b[0m");
    } else {
  ...
  if (p_evt->type == BLE_NUS_EVT_RX_DATA) {
    ...
    if (p_evt->params.rx_data.length < sizeof(message)) {
      ...
      if (p_evt->params.rx_data.length >= 5 && message[0] == '1' &&
        ...
        {
          // NRF_LOG_RAW_INFO("\n\n\x1b[1;36m--- Comando 10 recibido: Solicitar
          // último historial al periférico\x1b[0m");

          // // Comando a enviar al periférico
          // uint8_t  command_to_peripheral = 0x08; // Comando 08 para solicitar
          // el último historial uint16_t command_length        = 1;

          // // Enviar el comando al periférico a través del cliente NUS
          // ret_code_t err_code =
          // app_nus_client_send_command(&command_to_peripheral,
          // command_length); if (err_code == NRF_SUCCESS)
          // {
          //     NRF_LOG_RAW_INFO("\nComando 08 enviado al periférico para
          //     solicitar el último historial.\n");
          // }
          // else
          // {
          //     NRF_LOG_RAW_INFO("\nError al enviar el comando 08 al
          //     periférico: 0x%X\n", err_code);
          // }
          // break;
        }
        ...
        {
          NRF_LOG_RAW_INFO("\n\n\x1b[1;36m--- Comando 11 recibido: Solicitar "
                           "registro de historial por ID\x1b[0m");
          if (p_evt->params.rx_data.length >
              5) // Verifica que haya datos suficientes
          {
            // Extrae el ID del registro como string (todo lo que sigue después
            // de "11111")
            size_t id_len = p_evt->params.rx_data.length - 5;
            char id_str[8] = {
                0}; // Soporta hasta 7 dígitos, ajusta si necesitas más
            if (id_len < sizeof(id_str)) {
              ...
              err_code =
                  read_history_record_by_id(registro_id, &registro_historial);
              if (err_code == NRF_SUCCESS) {
                NRF_LOG_RAW_INFO(
                    "\nSolicitud de registro %u enviada correctamente.",
                    registro_id);
                // Imprime el registro de historial
                char titulo[41];
                snprintf(titulo, sizeof(titulo),
                         "Historial recibido \x1B[33m#%u\x1B[0m", registro_id);
                print_history_record(&registro_historial, titulo);
                NRF_LOG_FLUSH();
              } else {
                NRF_LOG_RAW_INFO("\nError al solicitar el registro %u: 0x%X",
        ...
        case 12: // Guardar un nuevo historial con valores inventados
          ...
          NRF_LOG_RAW_INFO("\n\n\x1b[1;36m--- Comando 12 recibido: Guardar "
                           "nuevo historial de prueba\x1b[0m");
          ...
          if (err_code == NRF_SUCCESS) {
            ...
            print_history_record(&nuevo_historial,
                                 "Historial de prueba guardado");
          } else {
            NRF_LOG_ERROR("Error al guardar historial de prueba: 0x%X",
                          err_code);
          }
          break;
        ...
        {
          NRF_LOG_RAW_INFO("\n\n\x1b[1;36m--- Comando 13 recibido: Mostrar e "
                           "incrementar record_id de history\x1b[0m");
...
void app_nus_server_ble_evt_handler(ble_evt_t const *p_ble_evt) {
  ...
  case BLE_GAP_EVT_CONNECTED:
    if (p_gap_evt->params.connected.role == BLE_GAP_ROLE_PERIPH) {
      ...
      NRF_LOG_RAW_INFO("\n\nReinicie el contador de encendido a 0");
      restart_on_rtc();
    }
    break;
  ...
  case BLE_GATTS_EVT_WRITE: {
    ...
    // NRF_LOG_RAW_INFO("\nWRITE handle: 0x%04X, cccd_handle: 0x%04X, "
    //                  "conn_handle: 0x%04X, emisor_conn_handle: 0x%04X",
    ...
    //   err_code = ble_nus_data_send(&m_nus, comando_conexion, &len,
    //                                m_emisor_conn_handle);
    //   if (err_code == NRF_SUCCESS) {
    //     NRF_LOG_RAW_INFO(
